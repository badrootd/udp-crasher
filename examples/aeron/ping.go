...
func Ping(aeronCtx *aeron.Context, cancel func()) {
	...
	handler := func(buffer *atomic.Buffer, offset int32, length int32, header *logbuffer.Header) {
		...
		if hist.TotalCount() == int64(*examples.ExamplesConfig.Messages) {
			logger.Infof("Ping done, cancelling Pong")
			cancel()
		}
	...
	for i := 0; i < warmupIt; i++ {
		...
		for true {
			...
			if ret > 0 {
				break
			} else if ret == aeron.BackPressured {
				idleStrategy.Idle(0)
			} else if ret < 0 {
